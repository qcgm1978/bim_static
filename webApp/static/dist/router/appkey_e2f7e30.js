...
var AppKeyRoute = Backbone.Router.extend({
	...
	//项目计划节点
	projectPlan: function projectPlan(projectId, planId) {
	...
	//获取项目最新版本
	fetchLastProjectVersionId: function fetchLastProjectVersionId(projectId, callback) {
	...
	//根据计划节点获取对应的构建
	fetchBuildIdByPlanCode: function fetchBuildIdByPlanCode(planCode, projectId, projectVersionId, callback) {
		App.Comm.ajax({
	...
	//模型分享
	shareModel: function shareModel(token) {
	...
	//解析token
	parseToken: function parseToken(token, callback) {
	...
	// 浏览变更模型与变更基准模型差异
	projectDifferBase: function projectDifferBase(projectCode, versionId) {
	...
	//浏览项目模型与标准模型差异
	projectDifferStd: function projectDifferStd(projectCode, versionId) {
		var _this = this;
		//初始化之前 验证
		this.beforeInit(function () {
	...
	//项目
	project: function project(projectCode, versionId) {
	...
	family: function family(libIid) {
		...
		this.beforeInit(function () {
			...
			_this2.resourceModel("famLibs", libIid);
		});
	},


	//标准模型库
	standardLibs: function standardLibs(modelCode, versionId) {
	...
	//资源库
	resourceModel: function resourceModel(type, projectId) {
	...
	//获取用户信息
	getUserInfo: function getUserInfo(fn) {
	...
	//格式化用户
	formatUser: function formatUser() {
	...
	//退出清除cookie
	cleanCookie: function cleanCookie() {
		//绑定beforeunload事件
		$(window).on('beforeunload', function () {
			//App.Comm.delCookie("token_cookie")
		});
	},
	projectByCode: function projectByCode(code, callback) {

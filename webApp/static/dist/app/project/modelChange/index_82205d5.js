;/*!/app/project/modelChange/js/collection.js*/
/**
...
;/*!/app/project/modelChange/js/index.es6*/
"use strict";
...
App.Index = {
	...
	//渲染模型
	renderModel: function renderModel(modelId) {
		var _this = this;
	...
	//渲染属性
	renderAttr: function renderAttr(modelId) {
	...
	bindPoint: function bindPoint(viewer) {
	...
	fetchChange: function fetchChange() {
		var that = this;
	...
	getDetail: function getDetail(comparisonId) {
	...
	//显示导航文字
	showNavBarText: function showNavBarText() {
		var _this2 = this;
		...
		App.Comm.ajax(data, function (data) {
			if (data.code == 0) {
				...
				App.Comm.ajax(data1, function (data) {
					if (data.code == 0) {
						data = data.data;
...
App.Project.Model = {
	...
	getInfo: Backbone.View.extend({
		...
		addDetail: function addDetail(model) {
			var data = model.toJSON();

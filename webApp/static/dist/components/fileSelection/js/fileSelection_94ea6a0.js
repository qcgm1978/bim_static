;/*!/components/fileSelection/es6/fileSelection.es6*/
"use strict";

(function ($, win) {

	var FileSelection = function FileSelection(options) {
	...
	FileSelection.prototype = {
		...
		setCookie: function setCookie(name, value) {
			var Days = 30,
			    exp = new Date();
			exp.setTime(exp.getTime() + Days * 24 * 60 * 60 * 1000), doMain = window.location.host.substring(window.location.host.indexOf("."));
		...
		//生成左侧树
		renderRightTree: function renderRightTree(data) {
		...
		//单个节点
		treeNode: function treeNode(item) {
		...
		//确认选择的文件
		enterSelect: function enterSelect() {
		...
		//根据id获取数据
		renderFile: function renderFile(data) {

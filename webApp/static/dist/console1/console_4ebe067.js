...
App.Console = {
  ...
  //质量监测
  qualityMonitoring: function qualityMonitoring() {
    var Settings = App.Console.Settings;
    //发起
    if (Settings.step == 1) {
  ...
  //获取项目列表
  getProjects: function getProjects(Type, cb) {
    $.ajax({
  ...
  apply: function apply(index, num, obj, type) {
  ...
  auditSheet1: function auditSheet1(type, selector, result) {
    $.ajax({
  ...
  twoApply: function twoApply(type, tagid, versionid) {
    $.ajax({
  ...
  urlfor: function urlfor(url1, url2, tagid, versionid) {
    $.ajax({
      url: url1
    }).done(function (data) {
